...
- **OpenCode configuration** is managed through `opencode.jsonc` (root-level) and `.chezmoidata/opencode.yaml` (data-driven templates).
...
# NEVER edit OpenCode config in target locations
...
# For OpenCode configuration - edit source files in chezmoi repo
vim opencode.jsonc                    # Root-level OpenCode config
vim .chezmoidata/opencode.yaml       # Data for OpenCode templates
vim dot_config/opencode/config.json.tmpl  # If using templated config
...
OpenCode configuration is managed through two primary files:

#### `opencode.jsonc` (Root-level)

- **Purpose**: Direct OpenCode configuration for LSP servers, formatters, and core settings
- **Location**: Repository root (`opencode.jsonc`)
- **When to edit**: Adding/modifying LSP servers, formatters, keybinds, or provider settings
- **Format**: JSON with comments (JSONC)

#### `.chezmoidata/opencode.yaml` (Data-driven)

- **Purpose**: YAML data that can feed into OpenCode template configurations
- **Location**: `.chezmoidata/opencode.yaml`
- **When to edit**: When using templated OpenCode configs that need dynamic data
- **Format**: YAML for easy data management

#### OpenCode Configuration Workflow:

1. **Direct config changes**: Edit `opencode.jsonc` for immediate OpenCode settings
2. **Template-driven config**: Edit `.chezmoidata/opencode.yaml` for data, create `dot_config/opencode/config.json.tmpl` for templated configs
3. **Apply changes**: Run `chezmoi apply` to deploy templated configs (if using templates)
4. **Validate**: Test OpenCode functionality after changes

**Note**: The root `opencode.jsonc` is used directly by OpenCode and doesn't require chezmoi apply. Template-based configs in `dot_config/opencode/` do require chezmoi apply.

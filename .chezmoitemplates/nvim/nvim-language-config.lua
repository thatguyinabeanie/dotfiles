...
return {
  ...
  treesitter = {
    parsers = {
{{- range $parsers_data.treesitter_parsers }}
      "{{ .name }}",
{{- end }}
    },
...
{{- end }}
    ...
    language_map = {
{{- range $formatter := $formatters_data.formatters }}
...
{{- end }}
    ...
    language_map = {
{{- $langMap := dict }}
{{- range $linter := $linters_data.linters }}

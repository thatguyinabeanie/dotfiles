...
This tmux configuration is designed for a modern terminal-based development
workflow with vim-like navigation, aesthetic enhancements, and tight integration
with other development tools. The configuration is modular and split across
...
All tmux commands are prefixed with `Ctrl+a` (changed from default `Ctrl+b`
for ergonomics).
...
| `Prefix + H/J/K/L`     | Resize panes    | Resize current pane in dir     |
...
| Key                | Action                     | Description                   |
| ------------------ | -------------------------- | ----------------------------- |
| `Prefix + c`      | `new-window -c ~/`         | Create new window (home dir)  |
| `Prefix + Ctrl-d` | `new-window -c chezmoi`    | New window at chezmoi dir     |
| `Prefix + Ctrl-e` | `new-window chezmoi + vim` | New window chezmoi + open vim |
...
| Key            | Action                   | Description          |
| -------------- | ------------------------ | -------------------- |
| `Ctrl-Shift-p` | Swap + select window -1 | Move window left     |
| `Ctrl-Shift-n` | Swap + select window +1 | Move window right    |
...
| Key            | Action                     | Description                  |
| -------------- | -------------------------- | ---------------------------- |
| `Prefix + v`  | `split-window -h`          | Vertical split (current dir) |
| `Prefix + \|` | `split-window -h`          | Vertical split (current dir) |
| `Prefix + -`  | `split-window -v`          | Horiz split (current dir)    |
| `Prefix + s`  | `split-window -v`          | Horiz split (current dir)    |
| `Prefix + %`  | `split-window -h`          | Vertical split (default)     |
| `Prefix + "`  | `split-window -v`          | Horiz split (default)        |
| `Prefix + \\` | `split-window -fh`         | Full-width vertical split    |
| `Prefix + _`  | `split-window -fv`         | Full-height horiz split      |
...
| `Prefix + Up/Down/Left/Right`| `select-pane`       | Select by direction|
...
| `Prefix + .`                   | `resize-pane -R 20` | Resize right 20 units   |
...
| `Prefix + Ctrl-Up/Down/L/R`   | `resize-pane`       | Resize by direction     |
| `Prefix + Alt-Up/Down/L/R`    | `resize-pane 5`     | Resize by dir (5 units) |
...
| `Ctrl-d/Ctrl-u`  | Half-page move   | Half page down/up            |
...
| `Ctrl-v` | `rectangle-toggle`           | Toggle rectangle select  |
...
| Key      | Action              | Description                      |
| -------- | ------------------- | -------------------------------- |
| `o`      | Open file/URL       | Copy selection and open default  |
| `Ctrl-o` | Open in nvim        | Copy selection and open in nvim  |
| `!`      | Copy to clipboard   | Copy without trailing newline    |
| `Y`      | Copy and paste      | Copy selection and paste now     |
...
| Key              | Action                | Description                  |
| ---------------- | --------------------- | ---------------------------- |
| `Prefix + P`     | Toggle logging        | Start/stop logging pane      |
...
| `Ctrl-Shift-p` | Move win left  | Swap window left and select it  |
| `Ctrl-Shift-n` | Move win right | Swap window right and select it |
...
- **Clipboard integration**: Enabled with `reattach-to-user-namespace`
  on macOS
...
3. **vim-tmux-navigator** - Seamless navigation between vim and tmux
...
The `tmux-fzf-url` plugin provides intelligent URL extraction and opening
capabilities:
...
4. Press Enter to open the selected URL with the system default app
...
The tmux configuration uses a sophisticated theming approach that integrates
with the broader system theme:

#### Theme Components

1. **Base Theme**: Catppuccin color palette with multiple variants
2. **Dynamic Switching**: Auto light/dark mode based on system settings
3. **Consistent Colors**: Shared color scheme across tmux, Neovim, terminal
4. **Custom Overrides**: Specific adjustments for visibility and aesthetics

#### Theme Files

- `tmux.theme.catppuccin.conf.tmpl`: Main theme config with Chezmoi
  templating
- Color variables are sourced from global Chezmoi data for consistency
- Supports multiple Catppuccin flavors (Latte, Frappe, Macchiato, Mocha)
...
# Magnet links
set -g @fzf-url-extra-filter 'grep -oE "(magnet:\?[^\s]*)"'
# Custom open binding
set -g @fzf-url-bind 'ctrl-o:execute-silent(open {})'
```
...
This comprehensive tmux setup provides a powerful, efficient, and
aesthetically pleasing terminal multiplexer experience that integrates
seamlessly with the broader development environment.

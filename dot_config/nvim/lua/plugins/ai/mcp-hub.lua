return {
  {
    ...
    config = function()
      -- Ensure mise tools are in PATH for Neovim (using Node.js major version 22)
      local mise_node_base = vim.env.HOME .. "/.local/share/mise/installs/node/"
      local mise_node_bin = nil

      -- Find any Node 22.x.x installation
      local uv = vim.loop
      local handle = uv.fs_scandir(mise_node_base)
      if handle then
        while true do
          ...
          if typ == "directory" and name:match("^22%.") then
            mise_node_bin = mise_node_base .. name .. "/bin"
            break
          end
        end
      end

...
This is a Neovim configuration directory within a chezmoi dotfiles repository. The setup is based
on [LazyVim](https://lazyvim.org/), a modern Neovim distribution that provides sensible defaults
and a plugin ecosystem. This configuration extends LazyVim with custom plugins, settings, and
...
**Note:** The complete list of installed LazyVim extras can be found in `.lazyvim.json`.
This config includes 51 extras covering AI, language support, debugging, testing,
...
- **Imports**: Prefer `require()` over `vim.api.nvim_get_runtime_file()`. Group requires
  logically
...
- **catppuccin.lua**: Catppuccin color scheme with dynamic light/dark mode switching
- **custom-theming.lua**: Custom theme overrides and integrations
- **comfy-line-numbers.lua**: Left-hand digit vertical motions
- **folke.lua**: Folke plugin suite configurations
- **render-markdown.lua**: Markdown rendering enhancements
- **treewalker.lua**: AST-aware navigation using Alt+hjkl
- **undotree.lua**: Undo history visualization

### Development Plugins (`lua/plugins/dev/`)

- **languages.lua**: Language-specific configurations
- **mason.lua**: LSP server and tool management
- **notebooks.lua**: Jupyter notebook integration (jupytext, molten, quarto)
- **treesitter.lua**: Syntax highlighting and text objects

### Git Plugins (`lua/plugins/git/`)

- **blame.lua**: Inline Git blame annotations
- **diffview.lua**: Side-by-side diff visualization with smart toggling
- **lazygit.lua**: LazyGit terminal integration
- **vim-fugitive.lua**: Core Git operations and staging

### UI Plugins (`lua/plugins/ui/`)

- **minimal-lualine.lua**: Minimal statusline configuration

### Utility Plugins (`lua/plugins/utilities/`)

- **chezmoi.lua**: Chezmoi template syntax highlighting and integration
- **golf.lua**: Code golf utilities
- **image.lua**: Inline image rendering via terminal graphics protocols
- **kulala.lua**: REST API testing from buffer
- **lazydocker.lua**: LazyDocker terminal integration
- **mcp-hub.lua**: Model Context Protocol integration
- **mermaid.lua**: Mermaid diagram support
- **pipeline.lua**: CI/CD pipeline utilities
- **smart-nav.lua**: Seamless vim/tmux/zellij pane navigation

## Jupyter Notebook Integration

### Overview

This configuration includes a comprehensive Jupyter Notebook environment that works
entirely within Neovim, eliminating the need for external browser windows or separate
applications.
...
1. **Opening Notebooks**: Open `.ipynb` files directly; jupytext creates
   editable `.qmd` buffer
...
This Neovim configuration provides a powerful, modern development environment that
integrates seamlessly with the broader dotfiles ecosystem while maintaining the
flexibility and extensibility that makes Neovim exceptional.
